<template>
  <div v-if="hasError">
    <div
      class="fixed flex flex-col items-center justify-center w-screen h-screen bg-background text-foreground z-[100] top-0 left-0">
      ...
      <a @click.prevent="refreshPage"
        class="mt-8 px-4 py-2 bg-background text-foreground rounded-md hover:bg-primary-foreground cursor-pointer">
        Go back home
      </a>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      errorStatus: null,
      errorMessage: '',
      component: null as VNode | null,
    };
  },
  created() {

    this.component = this.$slots.default ? this.$slots.default()[0] : null;
  },
